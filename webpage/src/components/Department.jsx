...
function Department() {
  ...
  const fetchdepartments = async () => {
    let url = `http://localhost:3001/department?page=${currentPage}&per_page=${perPage}`;
    if (search != "") {
      url += `&search=${search}`;
    }
    try {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        "http://localhost:3001/department/delete/" + id
      );
      fetchdepartments();
                  ...
                  <tr key={dep.dep_id}>
                    ...
                    <td>
                      ...
                      <button
                        ...
                        onClick={() =>
                          document
                            .getElementById("my_modal_" + dep.dep_id)
                            .showModal()
                        }
                      >
                        ลบ
                      </button>
                      <dialog id={`my_modal_${dep.dep_id}`} className="modal">
                        <div className="modal-box">
                          <h3 className="font-bold text-lg">ลบข้อมูลแผนก</h3>
                          <p className="py-4">
                            ต้องการลบข้อมูลแผนก {dep.dep_name} หรือไม่
                          </p>
                          <div className="modal-action">
                            <form method="dialog">
                              <button
                                className="btn btn-primary"
                                onClick={() => handleDelete(dep.dep_id)}
                              >
                                ยืนยัน
                              </button>
