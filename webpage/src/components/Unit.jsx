...
function Unit() {
  ...
  const fetchUnits = async () => {
    let url = `http://localhost:3001/Unit?page=${currentPage}&per_page=${perPage}`;
    if (search != "") {
      url += `&search=${search}`;
    }
    try {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        "http://localhost:3001/Unit/delete/" + id
      );
      fetchUnits();
                  ...
                  <tr key={Uni.unit_id}>
                    ...
                    <td>
                      ...
                      <button
                        ...
                        onClick={() =>
                          document
                            .getElementById("my_modal_" + Uni.unit_id)
                            .showModal()
                        }
                      >
                        ลบ
                      </button>
                      <dialog id={`my_modal_${Uni.unit_id}`} className="modal">
                        <div className="modal-box">
                          <h3 className="font-bold text-lg">
                            ลบข้อมูลหน่วยนับ
                          </h3>
                          <p className="py-4">
                            ต้องการลบข้อมูลหน่วยนับ {Uni.unit_name} หรือไม่
                          </p>
                          <div className="modal-action">
                            <form method="dialog">
                              <button
                                className="btn btn-primary"
                                onClick={() => handleDelete(Uni.unit_id)}
                              >
                                ยืนยัน
                              </button>
